...
%%
%% BoxLambda amendment by epsilon537.
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% }}}
...
0.32 & 06/07/2024 & epsilon537 & Added BoxLambda amendment.\\\hline
...
The BoxLambda variant of this SDSPI Controller adds the following registers
to the core:

\begin{table}[htbp]
\begin{center}\begin{reglist}
ISR     &{\tt 0x04} &  2 & R/W & Interrupt Status Register\\\hline
IEN     &{\tt 0x05} &  2 & R/W & Interrupt Enable Register\\\hline
\end{reglist}
\caption{I/O Peripheral Registers BoxLambda Amendment}\label{tbl:ioregs}
\end{center}\end{table}

The following bit positions are defined in the ISR and IEN registers:

\begin{itemize}
  \item Bit 0: IRQ\_BUSY\_BIT: This bit gets set when the controller goes from a busy to a non-busy state.
  \item Bit 1: IRQ\_CARD\_REMOVED\_BIT: This bit gets set when the controller detects that the SD card has been removed.
\end{itemize}

The SDSPI Controller core raises its {\tt o\_int} interrupt line when the ISR and IEN registers both have at least one bit set in the same bit position.
