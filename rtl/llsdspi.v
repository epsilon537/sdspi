    ...
    if (POWERUP_IDLE > 0) begin : WAIT_FOR_POWERUP
      ...
      always @(posedge i_clk)
        if (i_reset) begin
          ...
          powerup_hold    <= 1;
        end else if (powerup_hold) begin
          if (|powerup_counter) powerup_counter <= powerup_counter - 1;
          powerup_hold <= (powerup_counter > 0);
        end

`ifdef FORMAL
      always @(*) if (!f_past_valid) assume (powerup_counter > 2);
      always @(*) if (powerup_counter > 0) assert (powerup_hold);
`endif
      // }}}
    end else begin
    ...
    if (STARTUP_CLOCKS > 0) begin : WAIT_FOR_STARTUP
      ...
      always @(posedge i_clk)
        if (i_reset || powerup_hold) begin
          ...
          startup_hold    <= 1;
        end else if (startup_hold && r_z_counter && !o_sclk) begin
          if (|startup_counter) startup_counter <= startup_counter - 1;
          startup_hold <= (startup_counter > 0);
        end

`ifdef FORMAL
      always @(*) if (!f_past_valid) assume (startup_counter > 1);
      always @(*) if (startup_counter > 0) assert (startup_hold);
`endif
      // }}}
    end else begin
...
`ifdef FORMAL
  // {{{
  always @(*) if (powerup_hold) assert (startup_hold);
  // }}}
`endif
  ////////////////////////////////////////////////////////////////////////
  //
  ...
  // {{{
  always @(*)
    if (OPT_CONTINUOUS_CLOCK || powerup_hold) restart_counter = !powerup_hold;
    else begin
      restart_counter = 1'b0;

      if (startup_hold || !i_cs) restart_counter = 1'b1;
      else if (!OPT_SPI_ARBITRATION && byte_accepted) restart_counter = 1'b1;
      else if (OPT_SPI_ARBITRATION && r_state == LLSDSPI_IDLE) restart_counter = 1'b0;
      else if (OPT_SPI_ARBITRATION && r_state == LLSDSPI_WAIT && !bus_grant) restart_counter = 1'b0;
      else if (OPT_SPI_ARBITRATION && byte_accepted) restart_counter = 1'b1;
      else restart_counter = !r_idle;
    end
  // }}}
  ...
  always @(posedge i_clk) begin
    ...
    end else if (restart_counter) begin
    ...
    if (i_reset || (!CSN_ON_STARTUP && startup_hold)) begin
      ...
      if (!i_cs) begin
        ...
        if (byte_accepted) begin
          ...
          else r_state <= LLSDSPI_START + (OPT_CONTINUOUS_CLOCK ? 1 : 0);
        end
      end else if (r_state == LLSDSPI_WAIT) begin
        if (bus_grant) r_state <= LLSDSPI_START;
      end else if (byte_accepted) r_state <= LLSDSPI_START + 1;
      else if (o_sclk && r_state >= LLSDSPI_START) begin
        r_state <= r_state + 1;
        if (r_state >= LLSDSPI_END) r_state <= LLSDSPI_HOTIDLE;
      end

      if (startup_hold) o_cs_n <= 1;
    end
  // }}}

  // r_ireg
  // {{{
  always @(posedge i_clk) if (r_z_counter && !o_sclk) r_ireg <= {r_ireg[6:0], i_miso};
  // }}}

  // o_byte
  // {{{
  always @(posedge i_clk) if (r_z_counter && o_sclk && r_state == LLSDSPI_END) o_byte <= r_ireg;
  // }}}
  ...
  initial r_idle = 0;
  always @(posedge i_clk)
    if (startup_hold || i_reset) r_idle <= 0;
    else if (r_z_counter) begin
      if (byte_accepted) r_idle <= 1'b0;
      else if ((r_state == LLSDSPI_END) || (r_state == LLSDSPI_HOTIDLE)) r_idle <= 1'b1;
      else if (r_state == LLSDSPI_IDLE) r_idle <= 1'b1;
      else r_idle <= 1'b0;
    end
  // }}}
  ...
  always @(posedge i_clk)
    if (i_reset) o_sclk <= 1;
      ...
      if (OPT_CONTINUOUS_CLOCK) o_sclk <= !o_sclk;
  ...
  always @(posedge i_clk)
    if (i_reset) begin
      ...
      if (byte_accepted) begin
        o_mosi <= MOSI_INACTIVE_VALUE;
        if (o_cs_n && !OPT_CONTINUOUS_CLOCK) r_byte <= i_byte[7:0];
        else begin
          r_byte <= {i_byte[6:0], MOSI_INACTIVE_VALUE};
          o_mosi <= i_byte[7];
        end
      end else if (o_sclk && (!OPT_SPI_ARBITRATION || (bus_grant && r_state != LLSDSPI_WAIT))) begin
        r_byte <= {r_byte[6:0], MOSI_INACTIVE_VALUE};
        if (r_state >= LLSDSPI_START && r_state < LLSDSPI_END) o_mosi <= r_byte[7];
        else if (!i_cs) o_mosi <= MOSI_INACTIVE_VALUE;
      end
    end
  // }}}
  ...
  initial o_stb = 1'b0;
  always @(posedge i_clk)
    if (i_reset || startup_hold || !i_cs || !r_z_counter || !o_sclk) o_stb <= 1'b0;
    else o_stb <= (r_state >= LLSDSPI_END);
  // }}}

  // o_idle
  // {{{
  always @(*) begin
    if (OPT_CONTINUOUS_CLOCK) o_idle = (r_idle) && (r_z_counter) && (o_sclk);
    else o_idle = (r_idle) && (r_z_counter);
  end
  // }}}
  // }}}
  ////////////////////////////////////////////////////////////////////////////////
...
`endif
  ...
  always @(posedge i_clk)
    if (f_past_valid && $past(r_clk_counter != 0))
      ...
      else if (f_past_valid) assert ((r_clk_counter == 0) || (r_clk_counter == $past(i_speed)));

  //
  // And assumptions
  //
  always @(posedge i_clk) if (f_past_valid && (i_cs || !r_z_counter)) assume ($stable(i_speed));

  ////////////////////////////////////////////////////////////////////////
  //
  // Interface assumptions
  //
  always @(*) if (i_stb) `ASSUME(i_cs);

  always @(posedge i_clk)
    if (f_past_valid && $past(i_stb && !o_idle)) begin
      if ($past(i_reset || !i_cs)) `ASSUME(!i_stb);
      // else
      //  `ASSUME(i_stb);
      if ($past(f_past_valid) && (!$past(o_idle, 2))) `ASSUME($stable(i_byte));
    end else if (f_past_valid && $past(i_cs && !o_idle && !i_reset)) `ASSUME(i_cs);
  // assume(i_cs);

  always @(posedge i_clk)
    if (!f_past_valid)
      assert (o_cs_n == CSN_ON_STARTUP);
      else if ($past(bus_grant && !o_cs_n)) `ASSUME(bus_grant);
      else if ($past(!bus_grant && o_cs_n)) `ASSUME(!bus_grant);
  ...
  always @(posedge i_clk)
    if (f_past_valid && CSN_ON_STARTUP && $past(
            r_state == LLSDSPI_IDLE
        ) && $past(
            !i_reset
        ) && r_state == LLSDSPI_IDLE)
  ...
  always @(posedge i_clk)
    if ($past(!o_cs_n && bus_grant) && (!o_cs_n && bus_grant)) begin
      if (!$fell(o_sclk)) assume ($stable(i_miso));
  ...
  always @(*) assert (r_z_counter == (r_clk_counter == 0));

  always @(posedge i_clk) if (byte_accepted) fv_byte <= i_byte;

  always @(posedge i_clk) assert (r_state <= LLSDSPI_END);

  initial f_start_seq = 0;
  always @(posedge i_clk)
    if (i_reset || !i_cs) f_start_seq <= 0;
    else if (f_start_seq == 0) begin
      if (OPT_SPI_ARBITRATION) begin
        if (r_state == LLSDSPI_IDLE && byte_accepted) begin
          //  if (!o_cs_n && bus_grant)
          //    f_start_seq <= (f_next_seq == 0);
          //  //r_start_seq <= (!o_cs_n && bus_grant) ? 1:0;
        end else if (r_state == LLSDSPI_WAIT && r_z_counter) f_start_seq <= (bus_grant ? 1 : 0);
      end else if (byte_accepted) begin
        if (OPT_CONTINUOUS_CLOCK) f_start_seq <= (f_next_seq == 0) ? 2 : 0;
        else f_start_seq <= (f_next_seq == 0);
      end
    end else if (r_z_counter) begin
      ...
      if (byte_accepted) begin
        f_start_seq[17] <= 0;
        f_start_seq[16] <= 0;
      end else if (f_start_seq[17]) f_start_seq[17] <= 1;
    end

  always @(*) if (!OPT_SPI_ARBITRATION) assert (r_state != LLSDSPI_WAIT);

  always @(*)
    if (r_state == LLSDSPI_WAIT) begin
  ...
  always @(*)
    if (|(f_start_seq &{(8) {2'b01}}))
      assert (o_sclk);
      else if (|(f_start_seq &{(8) {2'b10}})) assert (!o_sclk);
  ...
  always @(posedge i_clk)
    if (i_reset || !i_cs) f_next_seq <= 0;
    else if ((|f_start_seq[17:16]) && byte_accepted) f_next_seq <= 1;
    else if ((|f_next_seq[16:15]) && byte_accepted) f_next_seq <= 1;
    else if (r_z_counter) begin
      f_next_seq <= f_next_seq << 1;
      // if (i_stb)
      //  f_next_seq[16] <= 0;
      // else
      if (f_next_seq[16]) f_next_seq[16] <= 1;
    end
  ...
  always @(*)
    if (|(f_next_seq &{(8) {2'b10}}))
      assert (o_sclk);
      else if (|(f_next_seq &{(8) {2'b01}})) assert (!o_sclk);
  ...
  always @(*) if (f_next_seq != 0) assert (f_start_seq == 0);
  ...
  always @(*)
    if ((f_start_seq == 0) && (f_next_seq == 0))
      assert ((r_state == LLSDSPI_IDLE) || (OPT_SPI_ARBITRATION && r_state == LLSDSPI_WAIT));

  always @(posedge i_clk)
    if (f_past_valid && !$past(i_reset) && $changed({o_sclk, o_mosi})) begin
      if ($past(i_cs)) begin
  ...
  always @(posedge i_clk)
    if (f_past_valid && !$past(i_reset) && !$past(r_z_counter)) begin
  ...
  always @(posedge i_clk) if (!$past(byte_accepted)) assume ($stable(f_rxdata));
  ...
  always @(*)
    if (!f_past_valid) begin
      if (STARTUP_CLOCKS > 0) `ASSUME(startup_hold);
      assert (r_z_counter);
      assert (!r_idle);
      assert (o_cs_n == CSN_ON_STARTUP);
      assert (o_sclk);
  ...
  always @(*) if (r_state == LLSDSPI_WAIT) assert (o_sclk && !o_cs_n);

  always @(*) if (!OPT_CONTINUOUS_CLOCK && o_cs_n && r_idle) assert (o_sclk);

  always @(*)
    if (startup_hold)
      assert (o_cs_n == CSN_ON_STARTUP);
      else if (!OPT_CONTINUOUS_CLOCK && o_cs_n) assert (o_sclk);


`ifdef VERIFIC
  always @(*) assert ($onehot0(f_next_seq));
  always @(*) assert ($onehot0(f_start_seq));
`endif
  ////////////////////////////////////////////////////////////////////////
  //
  // Cover checks
...
`ifdef LLSDSPI
  ...
  always @(posedge i_clk)
    if (i_reset || !i_cs) byte_count <= 0;
    else if (i_stb && !o_idle && (!(&byte_count))) byte_count <= byte_count + 1;
...
`endif
  ...
  always @(posedge i_clk)
    if (f_past_valid) begin
      if ($past(i_stb && !i_reset)) `ASSUME(i_cs);
    end

  always @(*) if (!CSN_ON_STARTUP && r_state == 0 && !o_cs_n) assume (!i_cs);
  ...
  always @(posedge i_clk)
    if (OPT_CONTINUOUS_CLOCK) begin
      ...
      if (!o_sclk && r_state == LLSDSPI_IDLE) assume (!i_stb);
    end
